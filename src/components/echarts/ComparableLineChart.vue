...
export default class ComparableLineChart extends Vue {
    id!: string;
    timeData!: number[];
    yDataList!: {
        [sourceName: string]: SingleData<number[]>;
    };
    visable!: boolean;
    hasMounted = false;
    name!: string;
    ...
    runRender(): void {
        if (!this.yDataList) return;
        const dataList = Object.entries(this.yDataList).map(entry => entry[1].data);
