...
global.roomResSvg=(res, allCnt)=>{
    ...
    return `<svg width="${len}px" height="8px"><rect width="500" height="10" fill="black"/>${svgs}</svg> ${_.padLeft(left,8)}/${_.padLeft(allCnt,8)} = ${_.padRight((left/allCnt*100).toFixed(2),5)}%`
}
...
if(!echarts)eval($.ajax({url:"https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js",async:false}).responseText);
var bgColor = '#2b2b2b';
...
return`
<t class="${divName}"  onclick="gotoRoom('${roomName}')">${roomName}</t><script>
    (() => {
        ...
        button.addEventListener("pointerenter", () => {
            ...
            tip.innerHTML = '<div id="${divNameShow}" style="height: 600px;width:600px;color:#000"/>';
            button.append(tip);
...
let pro = {
    ...
    showRoomRes(){
        ...
        result+=_.values(Game.rooms).filter(e=>e.my).map(room=>{
            ...
            let allCap = 0
            if(room.storage)allCap+=room.storage.store.getCapacity(RESOURCE_ENERGY)
            if(room.terminal)allCap+=room.terminal.store.getCapacity(RESOURCE_ENERGY)
            let free = 0
            if(room.storage)free += room.storage.store.getFreeCapacity()
            if(room.terminal)free += room.terminal.store.getFreeCapacity()
            if(free)res["empty"] = free

            let len = room.name.length
            let str=""
            for(let i=len;i<8;i++)str+=" ";
            return [roomResTips(room.name,res)+str+roomResSvg(res,allCap)+"<br>",allCap?free/allCap:0]
